# F: ヘルプ画面に AI 文言を追加 — 2026-06-13（最新）

**要件**: ヘルプに「AI は端末内で動作・無料・オフライン・写真や在庫は端末外に出ない、より賢い提案は設定で自前キー」を追記（オンデバイス実装済みなので正確に書ける）。

**実装 (TDD)**:
- `help_content.dart`（desktop/mobile 共有）の「使い方」ブロック直後に「AI について」`_Block` を追加（本文2段落 + 緑のプライバシー Callout）。
- l10n: helpAiEyebrow/Title/Body1/Body2/CalloutTitle/CalloutBody を en/ja/es に追加。
- テスト: help_desktop_view_test の①にAI見出し追加 + 専用テスト（オンデバイス説明・プライバシー）。
- 検証: **flutter analyze 0 / 全245テスト / macOS ビルド 成功**。
- これでオンデバイス AI 関連タスク（B1/B2/A/オンデバイス表示/C/E/F）が一通り完了。

---

# E: AI 非対応端末での AI 機能無効化＋案内 — 2026-06-13
