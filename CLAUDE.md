...
lib/
  ...
  core/
    ...
    shelf_life/             # 日持ち目安（食材名→日数の ShelfLifeTable + カテゴリ別フォールバック）
...
**`SyncService`**（macOS iCloud 実装済み） — クラウド同期共通インターフェース（`isAvailable` / `writeBackup` / `readBackup`）。
実装: macOS = `ICloudSyncService`（`lib/features/sync/service/icloud_sync_service.dart`）— platform channel `com.futo4.tsukaikiri/icloud_sync` 経由で `ICloudSyncPlugin.swift`（ubiquity コンテナ読み書き）を呼ぶ。バックアップは隠し領域 `backup/tsukaikiri_backup.json`（スキーマ版付き JSON・API キーは含めない）。
`SyncController`（`lib/features/sync/presentation/sync_controller.dart`）: Riverpod v3 `Notifier<SyncState>`。`backup()` / `restore()` / `applyRestore()` / `silentBackup()`。`BackupScheduler`（5秒デバウンス）と `autoBackupWatcherProvider` で在庫・設定変化時に自動バックアップ。
Android 実装予定: Google Drive App Data フォルダ（`drive.appdata` スコープ）。
...
- **日持ち目安データ（`ShelfLifeRule`）**: USDA FoodKeeper（2019-06 ミラー・CC0）+ 和食材補完をビルド時に `tool/shelf_life/generate.dart` で `assets/shelf_life/shelf_life_rules.json` に生成して同梱（実行時に API を呼ばない）。実行時は `ShelfLifeTable`（`lib/core/shelf_life/shelf_life_table.dart`、`main()` で起動時 load → `shelfLifeTableProvider` に注入）が食材名→日数を引く。照合は エイリアス完全一致→部分一致（最長優先）→normalizedKey→null。食材名でミスしたら `defaultExpiryFrom`（カテゴリ別フォールバック）。期限プレフィルは `expiryFromName(table, name, category, from)` 経由。データ更新・公式版差し替え手順は `tool/shelf_life/README.md`。
