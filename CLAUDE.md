...
1. `flutter create tsukaikiri`（macOS + iOS + Android ターゲット）
2. 在庫管理の最小版（ローカル DB のみ。AI・買い物リスト連携なし）— **i18n の土台（flutter_localizations + ARB）もこの段階で入れる**
3. `ShoppingListService` 抽象で買い物リスト連携（macOS から）
...
flutter run -d macos                  # macOS アプリとして実行
flutter test                          # 全テスト
...
実装: macOS/iOS = EventKit（Swift/ObjC platform channel、リマインダー）、Android = Google Tasks REST API（`google_sign_in` で `auth/tasks` スコープ取得）  
`UserSettings` には表示名でなく**識別子**（macOS/iOS=calendarIdentifier、Android=tasklist id）を保存する。

**`SyncService`** — クラウド同期共通インターフェース  
実装: macOS/iOS = iCloud Documents、Android = Google Drive App Data フォルダ（`drive.appdata` スコープ）  
...
- **API キー**: `flutter_secure_storage`（macOS/iOS=Keychain / Android=Keystore）にプロバイダごとに保存。
