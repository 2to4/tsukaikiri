...
class _OBSmallButtonState extends State<_OBSmallButton> {
  ...
  Widget build(BuildContext context) {
    ...
    final bg = _hovered ? AppColors.plentySoft : AppColors.card;
    const fg = AppColors.ink;
    final border = Border.all(color: AppColors.line, width: 1);
...
// M3 部品の「昇格」公開ラッパー
//
// settings_desktop_view.dart のファイルプライベート部品（_ApplianceCard 相当）を
// オンボーディングからも使えるよう公開名で再定義する。モバイルオンボーディング
// （onboarding_mobile_view.dart）からも参照する。
// （旧 OnboardingProviderGrid / OnboardingApiKeyCard はオンボーディングの
//   AI ステップを「オンデバイス可否表示のみ」に変更したため削除済み。）
// ─────────────────────────────────────────────────────────────
